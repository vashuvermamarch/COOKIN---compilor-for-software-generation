...
**🌟 LIVE DEPLOYMENT**: [https://cookin-app-930705731425.us-central1.run.app](https://cookin-app-930705731425.us-central1.run.app)
...
### 5. Arbitrary Javascript Execution & Custom Logic
* **HTML Widget Sandboxing**: For requests that require client-side logic (e.g. "Build a Calculator", "Create a Clock"), the LLM gracefully pivots away from the CRUD table schema and generates a fully encapsulated `html_widget` containing complete inline HTML/CSS/JS.
* This allows the system to seamlessly support both strict database forms and rich client-side interactive logic.

### 6. Automated Evaluation & Cost Framework
...
* **CRUD Execution**: Fill out dynamic input fields and submit forms. The system will write records directly to the physical SQLite database (`compiled_app.db`) on disk via the API endpoints and reload the tables dynamically in real-time.
* **Logic Apps (Calculators)**: Try prompting "Build a calculator" to see the engine deploy a fully functional client-side interactive Javascript application via the secure `html_widget` iframe!
